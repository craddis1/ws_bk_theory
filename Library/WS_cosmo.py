...
euclid_data = np.loadtxt('Library/snr_surveyparams.txt')
#load table data for SKA
SKAO1Data = np.loadtxt('Library/SKAO1Data.txt')
SKAO2Data = np.loadtxt('Library/SKAO2Data.txt')

class SurveyParams():
    def __init__(self,cosmo):
        """
           gets biases for a given survey
        """
...
class CosmologicalFunctions:
    def __init__(self,cosmo,survey_params={}):
        """
           takes in cosmology from CLASS and bias models and then can called to generate cosmology (f,P(k),P'(k),D(z) etc) and all other biases including relativstic parts
        ...
        if 'z_survey' in survey_params.keys():
            self.z_survey = survey_params['z_survey']
        else:
            self.z_survey  =  np.linspace(0.05,3)
        ...
        #define second order biases from b_1
    ...
    def get_class_powerspectrumNL(self,kk,zz): #h are needed to convert to 1/Mpc for k then convert pk back to (Mpc/h)^3
        return np.array([self.cosmo.pk(ki, zz) for ki in kk*self.h])*self.h**3
...
def numeric_ylm(func,l,m,params,derivs,betas,r=0,s=0,dfog=False,type_ = 0):
    """
    does numerical integration with double legendre guass integration implemented in int_gl_dbl()
    ...
    def integrand(phi,mu,*args):
        ...
        expression = func(*args,mu,phi,r=r,s=s)
    ...
    if type_ == 0:
        result = int_gl_dbl(integrand,params)
    elif type_ == 1:
        result = int_gl_dbl(integrand,params,derivs)
    elif type_ == 2:
        result = int_gl_dbl(integrand,params,betas)
    elif type_ == 3:
        result = int_gl_dbl(integrand,params,derivs,betas)
